...
// Earned title record — append-only catalogue of every title a user has won.
// Users collect many of these and choose which to display.
export type TitleCategory =
  | 'auto_current_season'   // generated by onPointsUpdate, revoked instantly on displacement
  | 'admin_historical';     // admin-granted (previous seasons / commemorative), permanent

export interface EarnedTitle {
  id: string;            // unique key, e.g. "{leagueId}_crown_champion" or "{userId}_{adminTitleId}"
  type: 'league_champion' | 'league_runner_up' | 'league_third'
       | 'global_crown_champion' | 'top_performer'
       | 'admin_custom';
  source_id: string;     // leagueId, 'global', or adminTitleId
  source_name: string;   // league name, 'Global Crown Championship', or admin-set label
  rank_label: '#1' | '#2' | '#3' | 'Global' | string; // string for admin-custom labels
  earned_at: string;     // ISO timestamp
  badge_variant: RankTitle; // maps to existing ChampionBadge styles
  points_at_earn: number;
  // NEW: reliability fields
  category: TitleCategory;
  is_temporary: boolean;          // true for current-season auto titles
  custom_label?: string;          // admin-set display name e.g. 'Plumbing League Champion 24-25'
  custom_emoji?: string;          // admin-set emoji override; default derived from rank
}

// Admin-defined title template — created in the admin panel, then granted to users.
// Example: { label: 'Plumbing League Champion', rank_label: '#1', emoji: '🏆' }
export interface AdminTitle {
  id: string;
  label: string;          // "Plumbing League Champion"
  rank_label: '#1' | '#2' | '#3' | string;
  emoji: string;          // 🏆 for #1, 🥈 for #2, 🥉 for #3 (admin can override)
  created_at: string;
...
export interface User {
  ...
  // Multi-title system
  earned_titles?: EarnedTitle[];
  displayed_title_id?: string | null;   // which title shows beside name everywhere
  pinned_title_ids?: string[];          // up to 5 IDs shown on public profile
}
