 ...
 * For Week 1, if admin has set a custom_draft_order, use that instead.
 */
export const getInitialDraftOrder = (members: LeagueMember[], week?: Week): string[] => {
    // If Week 1 has custom order set by admin, use it
    if (week?.week_number === 1 && week?.custom_draft_order && week.custom_draft_order.length > 0) {
        return week.custom_draft_order;
    }

    // Otherwise, fallback to automatic order (joined_at timestamp)
    return members
...
export const processTurnHandover = (week: Week, currentOrder: string[]): Partial<Week> => {
    ...
    if (isLastInOrder) {
        if (week.draft_round === 1) {
            // End of Round 1 -> WAIT FOR ADMIN to start Round 2
            return {
